...
#### 🎯 Objectif:
L’objectif de cette analyse est de comparer le profil défensif de quatre des meilleurs défenseurs centraux évoluant dans les cinq grands championnats européens sur la saison 2021-2022.
À l’aide de cinq indicateurs clés (tacles, interceptions, duels aériens, blocs défensifs, dégagements), nous cherchons à mettre en évidence les forces dominantes de chaque joueur et à déterminer si leur rôle varie en fonction des spécificités tactiques propres à leur championnat ou à leur équipe.

L’analyse est visualisée sous forme de radar chart, permettant une lecture globale et comparative des différents profils.
...
```{r radar_defenseurs_final, fig.width=7, fig.height=7}
library(dplyr)
library(fmsb)
...
```
#### 🧠 Interprétation du graphique:
Le radar chart permet de comparer les performances de Virgil van Dijk (Premier League), Marquinhos (Ligue 1), Kalidou Koulibaly (Serie A) et David Alaba (La Liga), chacun représenté par une couleur distincte. On observe par exemple que Van Dijk domine clairement les duels aériens, ce qui confirme son rôle de tour de contrôle défensive à Liverpool. Marquinhos propose un profil très équilibré, performant sur l’ensemble des dimensions, ce qui reflète sa régularité au PSG. Koulibaly est particulièrement actif dans les dégagements et les blocs, montrant une approche défensive plus réactive et physique. Alaba, en revanche, a un profil moins centré sur l’impact physique, ce qui correspond à son rôle souvent hybride (défenseur/milieu) dans un style de jeu plus construit à Madrid.
...
team_stats_2021_2022 <- team_stats_2021_2022 %>%
  mutate(
    Nb_buts_buteur = str_extract(Top.Team.Scorer, "\\d+$"),             # extrait le chiffre à la fin
    Buteur = str_remove(Top.Team.Scorer, "\\s*-\\s*\\d+$"),              # supprime le " - 15" du nom
...
```{r}
# 1️⃣ Nettoyage des montants investis
transfers_clean <- transfers_2022 %>%
  filter(!fee %in% c("Free", "Loan", "", NA)) %>%
  mutate(fee = str_remove_all(fee, "€|m|\\s"),
         fee = as.numeric(fee))  # en millions

# 2️⃣ Création d'un identifiant club unifié
...
# 3️⃣ Total des investissements par club (nettoyé)
investissements <- transfers_clean %>%
  mutate(club_clean = clean_name(club_to)) %>%
  group_by(club_clean) %>%
  summarise(invest_total = sum(fee, na.rm = TRUE))

# 4️⃣ Variation de points avec nom de club nettoyé
variation_points <- team_stats_2022_2023 %>%
  select(Squad, Pts) %>%
  rename(club = Squad, Pts_22_23 = Pts) %>%
  left_join(select(team_stats_2021_2022, Squad, Pts),
            by = c("club" = "Squad")) %>%
  rename(Pts_21_22 = Pts) %>%
  mutate(variation_pts = Pts_22_23 - Pts_21_22,
         club_clean = clean_name(club))

# 5️⃣ Fusion finale + filtre
df_invest <- left_join(variation_points, investissements, by = "club_clean") %>%
  filter(!is.na(invest_total))

# 6️⃣ Visualisation : scatter + droite de tendance
...
#### 🧠 Interprétation du graphique:
Ce graphique met en relation le montant investi par chaque club lors du mercato estival 2022 et la variation du nombre de points obtenue entre les saisons 2021-2022 et 2022-2023. Chaque point représente un club : l’axe horizontal indique le montant total investi (en millions d’euros), tandis que l’axe vertical reflète l’évolution des performances sportives, mesurée par la différence de points entre les deux saisons.

La droite de tendance (régression linéaire) présente une légère pente négative, ce qui indique que, dans notre échantillon, les clubs ayant le plus investi ont en moyenne connu une régression en termes de points.

Il est important de noter que le nombre de clubs présents dans ce graphique est limité. En effet, seuls les clubs figurant à la fois dans les tableaux de transferts et dans les données sportives des deux saisons peuvent être utilisés pour cette analyse, ce qui réduit la taille de l’échantillon. Toutefois, malgré ce nombre restreint, la visualisation reste utile : elle permet de soulever une tendance observable et invite à questionner l’efficacité réelle des investissements.

✅ **Conclusion** :  

Cette visualisation suggère que les investissements élevés ne garantissent pas nécessairement une amélioration des résultats sportifs. La légère tendance négative observée indique même qu’un recrutement massif peut s’accompagner d’une stagnation, voire d’une régression, en termes de performances.

Cela montre que le budget alloué au mercato n’est pas, à lui seul, un levier suffisant de réussite. La cohérence du projet sportif, la qualité du recrutement, l’intégration des joueurs ou encore la stabilité du staff peuvent avoir un impact bien plus significatif que la simple dépense brute. Ces résultats invitent à aller au-delà des montants investis pour comprendre les véritables ressorts de la performance sportive.
---
...
#### 🧠 Interprétation du graphique:
L’indice utilisé ici combine 4 dimensions de la création offensive :

Assists : passes menant directement à un but,

PasAss : passes clés (key passes),

PPA : passes vers la surface de réparation,

ScaPassLive : passes en jeu menant à une action de tir.

En divisant par le nombre de matchs joués, on obtient un indice normalisé, ce qui permet de comparer équitablement les joueurs.
