...
#### 🎯 Objectif:
Analyser quel championnat (ligue) est à l’origine du plus grand nombre de transferts durant l’été 2022. Cette analyse permet de comprendre les ligues les plus exportatrices de joueurs et d’identifier les marchés de talents actifs.

#### 🧪 Hypothèse :
Les ligues les plus réputées financièrement (ex : Premier League, Liga, Bundesliga) exportent plus de joueurs, mais certains championnats moins connus peuvent aussi être des pourvoyeurs importants de jeunes talents.


```{r}
# Préparation des données dans global.R ou en amont
transfers_clean <- transfers_2022 %>%
  filter(league_from != "") %>%
  mutate(fee_num = as.numeric(str_remove_all(fee, "[^0-9.]"))) %>%  # extraction numérique du montant (simplifié)
  mutate(fee_num = ifelse(is.na(fee_num), 0, fee_num))  # remplacer NA par 0
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      selectInput("ligueFilter", "Filtrer par ligue de départ :", 
                  choices = unique(transfers_clean$league_from),
                  selected = NULL,
                  multiple = TRUE)
    )
  ),
              ...
              sidebarLayout(
                sidebarPanel(
                  selectInput("countryFilter", "Filtrer par pays de départ :", 
                              choices = unique(transfers_clean$country_from),
                              selected = NULL,
                              multiple = TRUE),
...
server <- function(input, output, session) {
  ...
  # Filtrage dynamique pour volume transferts
  ...
  # Filtrage dynamique pour valeur marchande
  ...
  output$valeurPlot <- renderPlot({
    ...
    gg <- ggplot(df, aes(x = reorder(paste(league_from, "(", country_from, ")"), valeur_totale), y = valeur_totale, fill = valeur_totale)) +
      geom_col() +
      coord_flip() +
      scale_fill_gradient(low = "#FAD7A0", high = "#CA6F1E") +
      labs(title = "Top 20 ligues et pays par valeur totale des transferts - été 2022",
  ...
  output$valeurTable <- renderDT({
    df <- filtered_valeur() %>%
      ...
      arrange(desc(Valeur_totale))
    datatable(df, options = list(pageLength = 10), 
              colnames = c("Pays de départ", "Ligue de départ", "Valeur totale (€)"))
  })
...
#### 🎯 Objectif:
Analyser la répartition de la valeur totale des transferts selon le pays et la ligue d’origine des joueurs. Cela permet de mieux comprendre où se concentrent les investissements financiers et d’identifier les marchés les plus lucratifs.

#### 🧪 Hypothèse :
Les pays et ligues les plus riches ou les plus attractifs devraient générer des transferts avec des valeurs plus élevées. On s’attend donc à une concentration forte sur quelques ligues majeures.
...
#### 🧠 Interprétation du graphique:
- 🔵 Le graphique illustre clairement que certaines ligues et pays dominent le marché par la valeur financière des transferts.
...
✅ **Conclusion** :  
Les investissements sont concentrés dans un petit nombre de ligues majeures, confirmant l’hypothèse. Cette analyse financière complète l’analyse quantitative du volume des transferts.
